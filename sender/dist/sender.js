...
export async function startSender(videoStream, audioStream) {
    // Combine streams
    ...
    const rtpCapabilities = await new Promise(resolve => socket.emit('getRtpCapabilities', (data) => resolve(data)));
    await device.load({ routerRtpCapabilities: rtpCapabilities });
    // Create a producer transport
    const transportData = await new Promise(resolve => socket.emit('createProducerTransport', (data) => resolve(data)));
